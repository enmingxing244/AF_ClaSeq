...
def plot_distribution(df, parent_dir, metric_name, metric_type, num_cols, x_min, x_max, y_min, y_max, xticks, bin_step):
    ...
    for idx, iteration in enumerate(iterations):
        ...
        axes[row, col].xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: int(x) if x.is_integer() else f'{x:.1f}'))
        axes[row, col].set_yscale('log')
        axes[row, col].set_ylim(y_min, y_max)
        axes[row, col].yaxis.set_major_locator(ticker.LogLocator(base=10, numticks=10))
        axes[row, col].yaxis.set_minor_locator(ticker.LogLocator(base=10.0, subs=np.arange(2, 10) * 0.1, numticks=10))
