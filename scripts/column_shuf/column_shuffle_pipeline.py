...
# Import interaction analyzer from local module
...
class ColumnShufflePipeline:
    ...
    def _validate_unique_pairs_with_interactions(
        self,
        pdb_file: str,
        unique_pairs: List[Tuple[int, int]],
        state_name: str,
        ...
        """
        Validate unique pairs by checking for actual non-bonded interactions in PDB structure.

        Args:
            pdb_file: Path to PDB structure file
            unique_pairs: List of (res_i, res_j) tuples (1-indexed)
            state_name: Name of the state (for logging and output)
            output_dir: Directory to save interaction report
            chain_id: Chain ID in PDB structure (default: 'A')

        Returns:
            Tuple of (validated_pairs, interaction_details):
                - validated_pairs: List of pairs that have actual interactions
                - interaction_details: Dictionary mapping pair tuples to interaction info
        """
        self.logger.info(f"Validating {len(unique_pairs)} unique pairs for {state_name} using PDB structure")
        self.logger.info(f"PDB file: {pdb_file}")
        ...
        interaction_stats = {
            ...
            'pairs_with_interactions': 0,
            'pairs_without_interactions': 0,
            'interaction_types': {
                'hydrogen_bonds': 0,
                'salt_bridges': 0,
                'hydrophobic': 0,
        ...
        for res_i, res_j in unique_pairs:
            # Analyze interactions (residue numbers are already 1-indexed, matching PDB)
            # Use sidechain_only=True to focus on residue-specific interactions, not backbone H-bonds
            result = analyzer.analyze_pair(chain_id, res_i, chain_id, res_j, sidechain_only=True)

            if 'error' in result:
                self.logger.warning(f"Could not analyze pair ({res_i}, {res_j}): {result['error']}")
                continue

            # Check if pair has any interactions
            has_interaction = result.get('summary', {}).get('has_interaction', False)

            if has_interaction:
                validated_pairs.append((res_i, res_j))
                interaction_details[(res_i, res_j)] = result
                interaction_stats['pairs_with_interactions'] += 1
                ...
                interaction_stats['pairs_without_interactions'] += 1
                self.logger.debug(f"Pair ({res_i}, {res_j}) has no detectable interactions (min_dist={result.get('min_distance', 'N/A')} Å)")

        # Log summary
        self.logger.info(f"Interaction validation summary for {state_name}:")
        self.logger.info(f"  Total pairs analyzed: {interaction_stats['total_pairs']}")
        self.logger.info(f"  Pairs WITH interactions: {interaction_stats['pairs_with_interactions']}")
        self.logger.info(f"  Pairs WITHOUT interactions: {interaction_stats['pairs_without_interactions']}")
        self.logger.info(f"  Validation rate: {100 * interaction_stats['pairs_with_interactions'] / max(interaction_stats['total_pairs'], 1):.1f}%")

        # Log interaction type breakdown
        self.logger.info(f"  Interaction types found:")
        for itype, count in interaction_stats['interaction_types'].items():
            if count > 0:
                self.logger.info(f"    {itype}: {count} pairs")

        # Save detailed interaction report
        report_file = output_dir / f"{state_name}_interaction_validation.txt"
        with open(report_file, 'w') as f:
            f.write(f"{'=' * 80}\n")
            f.write(f"Interaction Validation Report: {state_name}\n")
            f.write(f"{'=' * 80}\n\n")
            f.write(f"PDB Structure: {pdb_file}\n")
            f.write(f"Chain ID: {chain_id}\n\n")

            f.write(f"SUMMARY\n")
            f.write(f"{'-' * 80}\n")
            f.write(f"Total pairs from contact map analysis: {interaction_stats['total_pairs']}\n")
            f.write(f"Pairs with validated interactions: {interaction_stats['pairs_with_interactions']}\n")
            f.write(f"Pairs without detectable interactions: {interaction_stats['pairs_without_interactions']}\n")
            f.write(f"Validation rate: {100 * interaction_stats['pairs_with_interactions'] / max(interaction_stats['total_pairs'], 1):.1f}%\n\n")

            f.write(f"Interaction Type Distribution:\n")
            for itype, count in interaction_stats['interaction_types'].items():
                f.write(f"  {itype}: {count} pairs\n")
            f.write(f"\n")
            ...
            for pair_idx, (res_i, res_j) in enumerate(validated_pairs, 1):
                result = interaction_details[(res_i, res_j)]

                f.write(f"Pair {pair_idx}: {result['residue1']} <-> {result['residue2']}\n")
                f.write(f"{'-' * 80}\n")
                f.write(f"Minimum distance: {result['min_distance']} Å\n")
                f.write(f"CB distance: {result['cb_distance']} Å\n")
                f.write(f"Total interactions: {result['summary']['total_interactions']}\n\n")

                # List each interaction type
                for interaction_type, data in result['interactions'].items():
                    if data:
                        f.write(f"{interaction_type.upper()}:\n")
            ...
            # List pairs that failed validation
            if interaction_stats['pairs_without_interactions'] > 0:
                f.write(f"\nPAIRS WITHOUT DETECTABLE INTERACTIONS\n")
                f.write(f"{'=' * 80}\n")
                f.write(f"These pairs showed contact probability difference in the sigmoid-normalized\n")
                f.write(f"contact map but do not have specific non-bonded interactions detectable by\n")
                f.write(f"geometric criteria (H-bonds, salt bridges, hydrophobic, pi-pi, pi-cation).\n")
                f.write(f"This may indicate distant or weak contacts.\n\n")

                for res_i, res_j in unique_pairs:
                    if (res_i, res_j) not in interaction_details:
                        f.write(f"  ({res_i}, {res_j})\n")

        self.logger.info(f"Saved interaction validation report: {report_file}")

        return validated_pairs, interaction_details
    ...
    def run_contact_map_construction(self) -> bool:
        ...
        try:
            ...
            # Validate unique pairs with actual interaction analysis
            output_dir = Path(self.config.general.base_dir) / "01_contact_maps"

            self.logger.info("=" * 80)
            self.logger.info("VALIDATING UNIQUE PAIRS WITH INTERACTION ANALYSIS")
            self.logger.info("=" * 80)

            # Validate state1 pairs
            validated_pairs_state1, interaction_details_state1 = self._validate_unique_pairs_with_interactions(
                pdb_file=self.config.general.state1_pdb,
            ...
            validated_pairs_state2, interaction_details_state2 = self._validate_unique_pairs_with_interactions(
                pdb_file=self.config.general.state2_pdb,
            ...
            self.logger.info("=" * 80)
            self.logger.info(f"State1 ({self.config.general.state1_name}):")
            self.logger.info(f"  Contact map pairs: {len(unique_pairs_state1)}")
            self.logger.info(f"  Validated pairs:   {len(validated_pairs_state1)}")
            self.logger.info(f"  Validation rate:   {100 * len(validated_pairs_state1) / max(len(unique_pairs_state1), 1):.1f}%")
            self.logger.info(f"State2 ({self.config.general.state2_name}):")
            self.logger.info(f"  Contact map pairs: {len(unique_pairs_state2)}")
            self.logger.info(f"  Validated pairs:   {len(validated_pairs_state2)}")
            self.logger.info(f"  Validation rate:   {100 * len(validated_pairs_state2) / max(len(unique_pairs_state2), 1):.1f}%")
            self.logger.info("=" * 80)
            ...
            # Save both raw and validated pairs
            unique_pairs_dict_raw = {
            ...
            self.logger.info(f"State1 - Raw pairs from contact map: {len(unique_pairs_state1)}, Validated with interactions: {len(validated_pairs_state1)}")
            self.logger.info(f"State2 - Raw pairs from contact map: {len(unique_pairs_state2)}, Validated with interactions: {len(validated_pairs_state2)}")
            self.logger.info(f"Using validated pairs for downstream MSA shuffling and prediction")
